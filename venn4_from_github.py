# slightly modified from https://github.com/ksahlin/pyinfor/blob/master/venn.py
'''
...
It is basic as it does not produce an area-weighted diagram.
'''
# FOR USE WITH `find_overlap_in_lists_with_Venn.py`
...
def venn4(data=None, names=None, fill="number", show_names=True, show_plot=True, **kwds):
    ...
    if show_plot:
        #pylab.show()
        pylab.savefig('venn4.png')

# MODIFIED FROM GITHUB TO SAVE FILE AS NAME
def venn4_w_fileNameSave(data=None, names=None, fill="number", show_names=True, show_plot=True, filename="venn4.png", **kwds):
